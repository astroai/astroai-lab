...
from astroai_lab.shell import hooks_sh_path, profile_sh_path
from astroai_lab.shell.session_env import export_shell
...
def env_install_shell(
    dest: Annotated[
        Path,
        typer.Argument(help="Destination directory (e.g. /etc/astroai-lab)."),
    ] = Path("/etc/astroai-lab"),
) -> None:
    """Install bundled profile.sh and hooks.sh for login shells.

    Examples:
        astroai-lab env install-shell /etc/astroai-lab
    """
...
def env_list(
    ...
    Examples:
        astroai-lab env list
        astroai-lab env list --json
        astroai-lab --json env list
    """
