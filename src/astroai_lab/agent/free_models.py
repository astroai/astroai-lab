...
def apply_cline(preset: str, *, dry_run: bool) -> str | None:
    if shutil.which("cline") is None:
        return "cline not installed — run: astroai-lab agent install cline"
    key = _openrouter_key()
    if not key:
        return (
            f"{OPENROUTER_KEY_ENV} not set — run: astroai-lab agent models free after exporting key"
        )
    model = PRESETS[preset]["openrouter"]
    if dry_run:
...
def free_models_guide() -> str:
    lines = [
        ...
        "  kilo     kilo-auto/free     astroai-lab agent install kilo && kilo auth",
        ...
        "",
        "Presets (astroai-lab agent models free --preset NAME):",
    ]
