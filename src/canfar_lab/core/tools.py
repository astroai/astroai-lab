...
def _canfar_lab_available() -> ToolInfo:
    ...
    try:
        ...
        return ToolInfo(
            ...
            available=True,
            path="(importable)",
            version=f"canfar-lab {__version__}",
        )
    except Exception:
        return info
...
def run_checks(*, warn_home_pct: int = 90) -> list[CheckItem]:
    ...
    # Required: git + canfar-lab. Project managers are recommended (strict requires them).
    git_info = tool_info("git", ("--version",))
    items.append(
        CheckItem("git", git_info.available, git_info.version or git_info.path or "missing")
    )
    lab_info = _canfar_lab_available()
    items.append(
        CheckItem(
            "canfar-lab",
            lab_info.available,
    ...
    if git.in_repo and git.uncommitted:
        items.append(
            CheckItem(
                "git_dirty",
                True,
                "uncommitted changes — commit or canfar-lab push before closing",
            )
        )
    elif git.in_repo:
