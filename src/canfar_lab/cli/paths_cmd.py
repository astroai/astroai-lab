...
def _paths_human(data: dict[str, str | None]) -> None:
    table = Table(title="canfar-lab paths", show_header=True, header_style="bold")
    table.add_column("Key", style="dim")
    table.add_column("Value")
    for key, value in data.items():
        table.add_row(key, value or "(unset)")
    console.print(table)
    ui.print_hint('  tip: `eval "$(canfar-lab env export)"` refreshes session env vars')


def _tools_human(tools: list[ToolInfo]) -> None:
    table = Table(title="canfar-lab tools", show_header=True, header_style="bold")
    ...
    if missing:
        ...
        ui.print_hint("  agents: `canfar-lab agent install --list`")


def _check_human(items: list[CheckItem]) -> None:
    table = Table(title="canfar-lab check", show_header=True, header_style="bold")
...
def register(app: typer.Typer) -> None:
    ...
    def check_cmd(
        ...
        if ok:
            ...
            ui.print_hint("  canfar-lab doctor --json")
            raise typer.Exit(1)
