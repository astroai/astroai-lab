#!/bin/bash
# Session reminders and exit hooks for CANFAR lab (interactive shells only).

__canfar_lab_state_dir() {
    echo "${CANFAR_LAB_CONFIG_DIR:-${HOME}/.canfar/lab}"
}

__canfar_lab_scratch_dir() {
    echo "${TMP_SCRATCH_DIR:-${CANFAR_LAB_DEFAULT_SCRATCH_DIR:-/scratch}}"
}
...
__canfar_lab_quota_reminder() {
    ...
    local _used_pct
    _used_pct="$(__canfar_lab_quota_used_pct "${HOME}")"
...
__canfar_lab_on_exit() {
    __canfar_lab_auto_archive
    if [[ -n "${__CANFAR_LAB_PRIOR_EXIT_TRAP:-}" ]]; then
        eval "${__CANFAR_LAB_PRIOR_EXIT_TRAP}"
    fi
}

if [[ -t 1 ]]; then
    __CANFAR_LAB_PRIOR_EXIT_TRAP="$(
        trap -p EXIT 2>/dev/null | sed -n "s/^trap -- '\(.*\)' EXIT\$/\1/p" || true
    )"
    if [[ -z "${__CANFAR_LAB_PRIOR_EXIT_TRAP}" || "${__CANFAR_LAB_PRIOR_EXIT_TRAP}" == "__canfar_lab_on_exit" ]]; then
        unset __CANFAR_LAB_PRIOR_EXIT_TRAP
    fi
    trap __canfar_lab_on_exit EXIT
fi

if [[ -t 1 ]]; then
    if [[ -z "${PROMPT_COMMAND:-}" ]]; then
        PROMPT_COMMAND="__canfar_lab_scratch_reminder; __canfar_lab_quota_reminder"
    else
        PROMPT_COMMAND="${PROMPT_COMMAND}; __canfar_lab_scratch_reminder; __canfar_lab_quota_reminder"
    fi
fi
