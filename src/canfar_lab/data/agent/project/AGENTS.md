...
Search: `rg`, `fd`, `sg` (ast-grep skill). View files: `peek <path>` (markdown/text/archives) or `bat`/`less`. Help: `canfar-lab guide`, `canfar-lab status --json`, `canfar-lab paths`, `canfar-lab tools`, `canfar-lab check`.
