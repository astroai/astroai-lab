...
def lab_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.delenv("CANFAR_LAB_WORK_DIR", raising=False)
    monkeypatch.delenv("CANFAR_LAB_SCRATCH_DIR", raising=False)
    monkeypatch.delenv("CANFAR_LAB_SAVE_DIR", raising=False)
    monkeypatch.delenv("TMP_SRC_DIR", raising=False)
    return home


def test_config_dirs_under_canfar(lab_home: Path) -> None:
    assert config_dir() == lab_home / ".canfar" / "lab"
    assert saves_dir() == lab_home / ".canfar" / "lab" / "saves"


def test_work_dir_from_canfar_lab_env(lab_home: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    work = lab_home / "work"
    work.mkdir()
...
def test_resolve_paths(lab_home: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert paths.config_dir == lab_home / ".canfar" / "lab"
