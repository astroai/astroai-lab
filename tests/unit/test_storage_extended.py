...
def test_stage_data_dry_run(tmp_path: Path) -> None:
    ...
    dst = tmp_path / "dst"
    src.mkdir()
    with patch("astroai_lab.core.storage.rsync_copy") as mock:
...
def test_rsync_missing_tool(tmp_path: Path) -> None:
    ...
    with patch("astroai_lab.core.storage.shutil.which", return_value=None):
...
def test_arc_project_statuses_marks_cwd() -> None:
    ...
    with patch("astroai_lab.core.storage.find_arc_project_root", return_value=foo):
        with patch("astroai_lab.core.storage.list_arc_projects", return_value=[bar, foo]):
            with patch("astroai_lab.core.storage.df_line", return_value=q) as mock_df:
                with patch("astroai_lab.core.storage.read_acl_groups", return_value=[]):
                    with patch("astroai_lab.core.storage.project_access", return_value="rw"):
                        with patch("astroai_lab.core.storage.list_gms_groups", return_value=None):
